...
bool Parser::validateRequestTree(const pt::ptree tree) const
{
  ...
  if(id == codes.defaultId){
    if(reque){
      string response = to_json(id, codes.defaultId);
      reque->push(response);
    }
    return false;
  }
  ...
  switch (request_type) {
    ...
    case codes.webRequestType:
      ...
      if (validateHost(host)){
        if(reque){
          string response = to_json(id, codes.defaultHost);
          reque->push(response);
        }
        return false;
      }
      ...
      if (validateProtocol(p)){
        if(reque){
          string response = to_json(id, codes.defaultProtocol);
          reque->push(response);
        }
        return false;
      }
      ...
      if (validateMethod(m)){
        if(reque){
          string response = to_json(id, codes.defaultMethod);
          reque->push(response);
        }
        return false;
      }
      ...
      if (validateReference(ref)){
        if(reque){
          string response = to_json(id, codes.defaultReference;
          reque->push(response);
        }
        return false;
      }
    ...
    case codes.cppRequestType:
      ...
      if (validateAdress(git)){
        if(reque){
          string response = to_json(id, codes.defaultGit);
          reque->push(response);
        }
        return false;
      }
      ...
      if (validateTarget(target)){
        if(reque){
          string response = to_json(id, codes.defaultTarget);
          reque->push(response);
        }
        return false;
      }
    ...
    default:
    if(reque){
      string response = to_json(id, codes.temporary);
      reque->push(response);
    }
      return false;
  }
